	<div class="content-real">
		<div id="blog">
			...
			<?php foreach($posts as $post): ?>
				...
				<article class="post featured">
				<header>
					<h4 class="title"><?= $post->message ?>
					</h4>
					<div class="meta">
						<time class="published"><?= mdate('%n %M %Y %H:%i:%s',human_to_unix($post->date));?></time>
						<span class="author">Asked by <?= $post->name;?></span>
					</div>
				</header>
				<p><?= $post->answer;?></p>
			</article>
			<?php endforeach; else: ?>
				<h2>No questions yet! Try to search <a href="http://41days.org/search/<?= $cari ?>">here</a> and <a href="http://41days.org/search/<?= $cari ?>">here</a> too!</h2>
			<?php endif;?>
