<script>
	function delete_friend($id) {
		...
		var id = $id;
		if (check == true) {
			window.location.href = "<?=base_url()?>friends/delete_friend/".concat(id);
		}
		else {
			return false;
		}
...
<div class="card-panel white">
	...
	<?php if( $query != '' ): foreach($query as $post): ?>
		<?= form_open('friends/friends/'.$post->id);?>
		...
		<div class="input-field">
			<label>Website</label>
			<input type="url" name="website" value="<?= $post->website ?>" required/>
		</div>
		...
		<?php if( isset($statuses) && $statuses): foreach($statuses as $status): if($status->alias != ""):?>
			...
			<label for="status-<?= $status->id;?>" style="margin-right:1em"><?= $status->alias;?></label>
	...
	<table class="highlight striped">
		...
		<tr>
			<?php if( isset($categories) && $categories): foreach($categories as $category): ?>
				...
				echo '<td>
				<a class="waves-effect waves-light btn red darken-4" href="'.base_url().'friends/friends/'.$category->id.'">update</a>
