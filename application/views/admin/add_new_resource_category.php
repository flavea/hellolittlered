<script>
	function delete_category($id) {
		...
		var id = $id;
		if (check == true) {
			window.location.href = "<?=base_url()?>admin/delete_resource_category/".concat(id);
		}
		else {
			return false;
		}
...
<?php if( $query != '' ): foreach($query as $post): ?>
		<?php echo form_open('admin/add-new-resource-type/'.$post->type_id);?>
		<h2 style="margin: .2em 0 1em 0" class="red-text text-darken-4">Update Category</h2>

		<input type="hidden" name="id" value="<?php echo $post->type_id ?>" >

		<div class="input-field">
			<label>Category Name</label>
			<input type="text" name="category_name" value="<?php echo $post->type_name ?>" required />
		</div>

		<div class="input-field">
			<label>Slug</label>
			<input type="text" name="category_slug" value="<?php echo $post->type_slug ?>" required />
		</div>
	...
	<table class="highlight striped">
		...
		<tr>
			<?php if( isset($categories) && $categories): foreach($categories as $category): ?>
				...
				echo '<td>
					<a class="waves-effect waves-light btn red darken-4" href="'.base_url().'admin/add_new_resource_type/'.$category->type_id.'">edit</a>
